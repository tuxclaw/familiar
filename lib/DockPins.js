...
function rename(pins, folderId, name) {
    var next = clone(pins)
    next.forEach(function(pin) {
        if (pin.type === "folder" && pin.id === folderId) pin.name = name
    })
...
function take(pins, id, folderId) {
    if (folderId) {
        ...
        if (folder.items.length < 2)
            pins.splice.apply(pins, [pins.indexOf(folder), 1].concat(folder.items))
        return id
    }
...
var widgetIds = ["omarchy.weather", "omarchy.audio", "omarchy.microphone", "omarchy.bluetooth",
    "omarchy.network", "omarchy.power", "omarchy.clock", "omarchy.monitor", "omarchy.tailscale"]

function widgetState(widgets, side) {
    if (!Array.isArray(widgets) || (side !== "left" && side !== "right"))
        throw new Error("invalid widget state")
    var result = []
    widgets.forEach(function(id) {
        if (typeof id !== "string" || widgetIds.indexOf(id) < 0) throw new Error("unknown widget")
        if (result.indexOf(id) < 0) result.push(id)
    })
    return { widgets: result, widgetSide: side }
}

// QML sequences expose indexed values but need not pass Array.isArray.
// Copy only list-shaped objects; strings and missing lists remain invalid.
function sequenceArray(list) {
    if (!list || typeof list !== "object" || typeof list.length !== "number"
        || !isFinite(list.length) || list.length < 0 || Math.floor(list.length) !== list.length)
...
function document(data) {
    if (!data || Array.isArray(data) || typeof data !== "object"
        || ["pins", "pins,widgetSide,widgets"].indexOf(Object.keys(data).sort().join(",")) < 0)
        throw new Error("invalid dock document")
    var state = Object.keys(data).length === 1 ? widgetState([], "right") : widgetState(sequenceArray(data.widgets), data.widgetSide)
    return { pins: normalize(sequenceArray(data.pins)), widgets: state.widgets, widgetSide: state.widgetSide }
}
